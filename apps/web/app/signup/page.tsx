...
/**
 * Calculate effective monthly price for a channel given selected modifiers.
 * - base price (monthlyPrice)
 * - + fiveDevicesAddonPrice when devices === 5
 * - - withAdsDiscount when hasAds === true
 */
function calcChannelPrice(
  ch: Channel,
  devices: number,
  hasAds: boolean
): number {
...
export default function SignupPage() {
  ...
  const filteredChannels = useMemo(() => {
    ...
    if (channelSearch.trim()) {
      ...
      filtered = filtered.filter((c: Channel) =>
  ...
  const toggleChannel = (id: string) => {
    ...
    const existing = selectedChannels.find(
  ...
  const monthlyTotal = useMemo(() => {
    ...
    // selectedChannels is now an array of ChannelConfig objects.
            ...
            {dataLoading ? (
              ...
              <>
                ...
                <div className="text-center space-y-2 pt-4">
                  ...
                  <p className="text-white/50">
                    Select bundles for the best value, or pick individual
                    channels
                  </p>
                </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                      {filteredChannels.map((ch: Channel) => {
                        const isSelected = selectedChannels.some(
                          (c: ChannelConfig) => c.subsiteId === ch.id
                        );
                        const chConfig = selectedChannels.find(
                          (c: ChannelConfig) => c.subsiteId === ch.id
                        );
                        const thumbUrl =
                              ...
                              <div className="flex items-center gap-3 mb-1">
                                <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-yellow-500/30 to-orange-500/30 flex items-center justify-center">
                                  <span className="text-lg">
                                    {bundle.name.includes("Kids")
                                      ? "🧒"
                                      : bundle.name.includes("Education")
                                      ? "📚"
                                      : "🎬"}
                                  </span>
                                </div>
                                <div>
                                  ...
                                  <p className="text-xs text-white/40">
                                    {bundle.bundleSubsites.length} channels for
                                    the whole family
                                  </p>
                                </div>
                              </div>
                              {bundle.description && (
                                <p className="text-sm text-white/50 mt-2 mb-3">
                                  {bundle.description}
                                </p>
                              )}
                              <div className="grid grid-cols-2 gap-x-8 gap-y-1">
                                {bundle.bundleSubsites.map(
                                  (bs: { subsite: Channel }) => (
                                    <div
                                      key={bs.subsite.id}
                                    ...
                                    >
                                      ...
                                      <span className="text-white/40">
                                        {bs.subsite.monthlyPrice
                                          ? formatPrice(bs.subsite.monthlyPrice)
                                          : ""}
                                      </span>
                                    </div>
                                  )
              ...
              <div className="flex gap-6">
                {selectedStreamPlan && (
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-sm font-medium bg-blue-500/20 text-blue-400 px-2 py-0.5 rounded">
                        Streams
                      </p>
                      <p className="text-xs text-white/50 bg-blue-500/10 px-2 py-0.5 rounded mt-0.5">
                        {selectedStreamPlan.name}
                      </p>
                    </div>
                  </div>
                )}
