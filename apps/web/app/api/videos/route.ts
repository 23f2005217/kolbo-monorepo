  ...
  try {
    ...
    for (const video of videos) {
      for (const asset of (video as any).assets || []) {
        if (
          asset.muxAssetId &&
          !asset.muxAssetId.startsWith("temp-") &&
          !asset.muxPlaybackId
        ) {
          assetsToUpdate.push({
            assetId: asset.id,
    ...
    for (const { assetId, muxAssetId } of assetsToUpdate.slice(0, 3)) {
      try {
        ...
        if (
          ...
          muxAsset.playback_ids.length > 0
        ) {
          const signedPlaybackId = muxAsset.playback_ids.find(
            (p) => p.policy === "signed"
          );
          const publicPlaybackId = muxAsset.playback_ids.find(
            (p) => p.policy === "public"
          );
          ...
          for (const video of videos) {
            for (const asset of (video as any).assets || []) {
              if (asset.id === assetId) {
                ...
                asset.durationSeconds = muxAsset.duration
                  ? Math.round(muxAsset.duration)
                  : null;
                asset.status = "ready";
              }
    ...
    const videosWithUrls = await Promise.all(
      videos.map(async (video: any) => {
        const horizontalImage = video.images?.find(
          (img: any) => img.imageType === "horizontal"
        );
        if (horizontalImage) {
        ...
        const primaryAsset =
          video.assets?.find((a: any) => a.isPrimary) || video.assets?.[0];
        ...
        if (policy === "signed") {
          try {
            const signedThumb = await getSignedThumbnailUrl(playbackId, {
              width: 200,
              height: 120,
              fitMode: "smartcrop",
            });
        ...
        return {
          ...video,
          muxThumbnailUrl: `https://image.mux.com/${thumbId}/thumbnail.png?width=200&height=120`,
        };
      })
    );
