...
import { Plus, Search, MoreHorizontal, Play } from "lucide-react";
...
export default function PlaylistsPage() {
  ...
  const handleEdit = (id: string) => {
    router.push(`/content/playlists/${id}`);
  };
  ...
  const handleCreatePlaylist = () => {
    router.push("/content/playlists/new");
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">KolBo HQ</h1>
        ...
        >
          <div className="mt-2 p-2 divide-y">
            {filteredPlaylists.map((playlist, index) => (
              <div
                key={playlist.id}
                className="grid grid-cols-[40px_80px_1fr_150px_200px_40px] items-center px-4 py-4 hover:bg-muted/50 transition-colors group rounded-md"
